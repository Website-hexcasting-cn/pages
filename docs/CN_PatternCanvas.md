...
**参数**：
- `PatternCanvasVirtual`：虚拟画布对象
- `PatternData`：样式配置
- `HighlightPoint`：当前高亮的点（可选）

**内部实现**：
1. 清空画布
2. 计算可见区域的虚拟坐标范围
3. 绘制六边形点阵（偶数行偏移 `Spacing/2`）
4. 绘制已保存的图案线段
5. 绘制当前绘制中的路径预览
6. 绘制高亮点（放大显示）
7. **debug 模式**：在每个点旁标注虚拟坐标 `(i,j)`
...
**内部实现**：
1. 调用 `IsMouseOverPoint` 检测
2. 若靠近点，发送 `PatternCanvasMouseMove` 消息并设置高亮
3. 若未靠近，清除高亮
4. 触发 `RefreshPatternCanvas()`
...
## 具体内嵌方法
创建id为PatternCanvas的canvas元素并且加载脚本
例子
```html
