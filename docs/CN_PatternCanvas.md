...
PatternCanvas 现已采用 ES6 模块化架构，代码被拆分为以下模块：

- **Utils.js** - 工具函数（颜色转换、角度计算等）
- **VirtualCanvas.js** - 虚拟画布管理
- **Renderer.js** - 渲染相关函数
- **GridManager.js** - 网格管理
- **PatternProcessor.js** - 图案处理
- **MessageHandler.js** - 消息处理
- **EventHandler.js** - 事件处理
- **index.js** - 主入口文件

所有功能保持向后兼容，只需更新引用方式即可。
...
PatternData = {
    Point: {
        InnerColor: "#1890ff",      // 点中心颜色
        OuterColor: "#bae7ff",      // 点外围颜色
        Size: 5,                    // 点的大小
        SelectedSize: 1             // 高亮点放大倍数
    },
    Grid: {
        Spacing: 80                 // 点阵间距（像素）
    },
    Line: {
        Color: "#009dffff",         // 图案线条颜色
        Width: 4                    // 图案线条宽度
    },
    Mouse: {
        Range: 1.5,                 // 鼠标靠近显示点半径（格距倍数）
        FadeRate: 0.7               // 淡出速率
    },
    Mode: {
        FreePainting: false,        // 自由绘制模式
        ShowNearMouse: true,        // 只显示鼠标附近的点
        FadeWithDistance: true      // 根据距离淡出
    },
...
| 属性路径 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| Point.InnerColor | string | "#1890ff" | 点中心颜色 |
| Point.OuterColor | string | "#bae7ff" | 点外围颜色 |
| Point.Size | number | 5 | 点的大小 |
| Point.SelectedSize | number | 1 | 高亮点放大倍数 |
| Grid.Spacing | number | 80 | 点阵间距（像素） |
| Line.Color | string | "#009dffff" | 图案线条颜色 |
| Line.Width | number | 4 | 图案线条宽度 |
| Mouse.Range | number | 1.5 | 鼠标靠近显示点半径（格距倍数） |
| Mouse.FadeRate | number | 0.7 | 淡出速率 |
| Mode.FreePainting | boolean | false | 自由绘制模式（可跳点绘制） |
| Mode.ShowNearMouse | boolean | true | 只显示鼠标附近的点（性能优化） |
| Mode.FadeWithDistance | boolean | true | 根据距离淡出显示 |
...
PatternData.Mode 对象包含以下配置：

| 配置项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| FreePainting | boolean | false | 自由绘制模式，可跳点绘制；false 时只能移动到相邻点 |
| ShowNearMouse | boolean | true | 只渲染鼠标周围的点（性能优化） |
| FadeWithDistance | boolean | true | 根据距离淡出显示 |
